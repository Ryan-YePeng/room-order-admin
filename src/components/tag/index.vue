...
export default {
  ...
  methods: {
    ...
    // 点击标签改变激活菜单和面包屑
    tabsClick(tag) {
      this.$storeSet("setBreadcrumb", tag.index);
    ...
    tabsRemove(obj) {
      ...
      // 获取当前激活菜单
      let active = this.$storeGet.active;
      if (active !== title) {
        // 删除
        this.$storeSet("deleteTags", index);
        return;
      }
      let nextIndex;
      ...
      // 改变激活菜单
      this.$storeSet("setActive", nextObj.title);
      // 改变面包屑
      this.$storeSet("setBreadcrumb", nextObj.index);
      // 删除
      this.$storeSet("deleteTags", index);
    },
    // 初始化当前滚动高度
    initialScrollTop(isIntial = false) {
      isIntial
        ? (document.querySelector(
            ".top"
          ).scrollTop = this.$storeGet.scrollTop)
        : setTimeout(() => {
    ...
    // 刷新子路由
    refresh() {
      let activeMenu = this.$storeGet.active;
